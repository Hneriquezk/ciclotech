...
$paginaTitle = "Clientes - Ciclotech";
include('config.php');

// ====== EXCLUIR CLIENTE ======
if (isset($_GET['excluir'])) {
    $id = intval($_GET['excluir']);
    // Exclui o cliente e todos os pedidos dele (opcional)
...
<head>
    <meta charset="UTF-8">
    <title>Clientes - Ciclotech</title>
...
.tabela-clientes tr:last-child td {
    border-bottom: none;
}
...
<body>
    ...
    <div class="content-area">
        <h2 class="titulo-clientes">Clientes Cadastrados</h2>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= isset($row['data_cadastro']) ? date('d/m/Y H:i', strtotime($row['data_cadastro'])) : '—' ?></td>
                            <td>
                                <a href="clientes.php?excluir=<?= $row['id'] ?>" class="btn-excluir" onclick="return confirm('Deseja excluir este cliente?')">Excluir</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
